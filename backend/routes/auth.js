...
// Require JWT_SECRET to be explicitly set - never use a fallback in production
...
router.post('/signup', async (req, res) => {
    try {
        ...
        if (!/[A-Z]/.test(password)) {
            return res.status(400).json({ ok: false, error: 'Password must contain at least one uppercase letter' });
        }
        if (!/[0-9]/.test(password)) {
            return res.status(400).json({ ok: false, error: 'Password must contain at least one digit' });
        }
