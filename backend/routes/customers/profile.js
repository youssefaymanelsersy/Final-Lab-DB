...
router.put('/:id', verifyCustomer, ensureSameCustomer, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const { username, first_name, last_name, email, phone, shipping_address } =
      req.body;
    ...
    // Check username uniqueness (allow same username for this same user)
    ...
    // Optional: if you allow email updates, also protect uniqueness
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      try {
        await pool.query('UPDATE customers SET avatar_url=? WHERE id=?', [
          publicUrl,
          id,
        ]);
      } catch (e) {
